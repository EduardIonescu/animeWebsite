...
import { ICharacters } from "../../../../constants/sectionInterfaces/charactersInterface";
      ...
      {voice_actors && voice_actors.length >= 1 && !isPage && (
        <div className="flex gap-2">
          <div className="text-end">
            <h4 className="text-[14px] mb-1">{voice_actors[0].person.name}</h4>
        ...
        <ul className="flex flex-col items-end">
          {voice_actors.map((voiceActor, i) => (
            <li key={i} className="flex gap-2">
              <div className="text-end">
                <h4 className="text-[14px] mb-1">{voiceActor.person.name}</h4>
                <p className="text-[11px]">{voiceActor.language}</p>
              </div>
              <div
                className="relative shrink-0 w-[44px] h-[67px] border-[1px]
							 border-black/20 dark:border-coolBlack"
